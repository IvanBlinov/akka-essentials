...
class FSMSpec extends TestKit(ActorSystem("FSMSpec")) with ImplicitSender with WordSpecLike with BeforeAndAfterAll with OneInstancePerTest {
...
object FSMSpec {
  ...
  case class Instruction(instruction: String) // message that VM will show on its "screen"
  ...
  class VendingMachine extends Actor with ActorLogging {
    ...
    def operational(inventory: Map[String, Int], prices: Map[String, Int]): Receive = {
      case RequestProduct(product: String) => inventory.get(product) match {
        case None | Some(0) =>
    ...
    when(WaitForMoney, stateTimeout = 1 second) {
      ...
      case Event(ReceiveMoney(amount), WaitForMoneyData(inventory, prices, product, money, requester)) =>
        ...
        if (money + amount >= price) {
          ...
          val remainingMoney = price - money - amount
          requester ! Instruction(s"Please insert $remainingMoney dollars")
          stay() using WaitForMoneyData(inventory, prices, product, money + amount, sender())
        }
    }

    whenUnhandled {
      case Event(_, _) =>
        sender() ! VendingError("Command not fount")
        stay()
    }
